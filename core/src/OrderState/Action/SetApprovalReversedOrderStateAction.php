...
class SetApprovalReversedOrderStateAction implements SetOrderStateActionInterface
{
    /**
    ...
    public function execute(string $payPalOrderId)
    {
        ...
        $this->changeOrderStateAction->execute(
            $order->id,
            $this->orderStateMapper->getIdByKey(OrderStateConfiguration::PS_CHECKOUT_STATE_CANCELED)
        );
    }
     ...
     * @return bool
     */
    private function hasBeenCanceled(Order $order): bool
    {
        return (bool) count(
            $order->getHistory(
                $order->id_lang,
