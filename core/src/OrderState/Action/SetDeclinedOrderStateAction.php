...
class SetDeclinedOrderStateAction implements SetOrderStateActionInterface
{
    /**
     ...
     * @return bool
     */
    private function orderHasErrorState(\Order $order): bool
    {
        return count($order->getHistory(
            $order->id_lang,
            $this->configuration->getInteger(OrderStateConfiguration::PS_CHECKOUT_STATE_ERROR)
        )) > 0 || count($order->getHistory(
            $order->id_lang,
            $this->configuration->getInteger(OrderStateConfiguration::PS_CHECKOUT_STATE_CANCELED)
        )) > 0;
    }
}
