...
class SetAuthorizedOrderStateAction implements SetOrderStateActionInterface
{
    /**
    ...
    public function execute(string $payPalOrderId)
    {
        ...
        if (!$order || !$order->id || $this->hasBeenPaid($order)) {
            return;
        }

        $this->changeOrderStateAction->execute(
            $order->id,
            (string) $this->orderStateMapper->getIdByKey(OrderStateConfiguration::PS_CHECKOUT_STATE_AUTHORIZED)
        );
    }
