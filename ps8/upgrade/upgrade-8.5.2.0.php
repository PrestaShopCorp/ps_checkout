...
/**
 * Update main function for module version 8.5.2.0
 *
 * @param Ps_checkout $module
 *
 * @return bool
 */
function upgrade_module_8_5_2_0(Ps_checkout $module)
{
    try {
        ...
        foreach ($shopsList as $shopId) {
            // Generate a support token for shops that don't have one yet
            $existingToken = Configuration::get('PS_CHECKOUT_SUPPORT_TOKEN', null, null, (int) $shopId);
            if (empty($existingToken)) {
                Configuration::updateValue(
                    'PS_CHECKOUT_SUPPORT_TOKEN',
            ...
            // Reset registration timestamp so re-registration is attempted on next admin page load
            Configuration::updateValue('PS_CHECKOUT_SUPPORT_REGISTERED_AT', '0', false, null, (int) $shopId);
        }
