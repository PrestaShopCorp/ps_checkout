...
class GenericClient
{
    ...
    private function call($method, array $options)
    {
        ...
        if (method_exists($client, 'getEmitter') && true === (bool) $this->getConfiguration(LoggerFactory::PS_CHECKOUT_LOGGER_HTTP, true)) {
        ...
        try {
            $response = $client->$method($this->getRoute(), $options);
