...
namespace Tests\Unit\Order\Payment\ValueObject;
...
class OrderPaymentIdTest extends TestCase
{
    ...
    {
        $valueObject = new OrderPaymentId(1);
        $this->assertEquals(1, $valueObject->getValue());
    }
    ...
    public function testInvalidValueThrowsException($value)
    {
        ...
        $this->expectExceptionCode(OrderPaymentException::INVALID_ID);
        $this->expectExceptionMessage(sprintf('Order payment id %s is invalid. Order payment id must be number that is greater than zero.', var_export($value, true)));
        new OrderPaymentId($value);
    }
