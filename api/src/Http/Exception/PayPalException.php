...
class PayPalException extends PsCheckoutException
{
    ...
    const INVALID_POSTAL_CODE = 163;

    const INVALID_POSTAL_CODE_LENGTH = 164;

    const MISSING_PREVIOUS_REFERENCE = 165;

    const MISSING_CRYPTOGRAM = 166;

    const PAYMENT_ORIGIN_NOT_ENABLED = 167;

    const PAYEE_PRICING_TIER_ID_NOT_ENABLED = 168;

    const INVALID_PAYEE_PRICING_TIER_ID = 169;

    const PAYEE_FX_RATE_ID_EXPIRED = 170;

    const PAYEE_FX_RATE_ID_CURRENCY_MISMATCH = 171;

    const INVALID_FX_RATE_ID = 172;

    const PLATFORM_FEES_NOT_SUPPORTED = 173;

    const INVALID_PLATFORM_FEES_ACCOUNT = 174;

    const UNSUPPORTED_INTENT_FOR_KYC_INCOMPLETE_PAYEE = 175;

    const INVALID_SHIPPING_OPTION_ID = 176;

    const INVALID_DELIVERY_ESTIMATE = 177;

    const INELIGIBLE_SHIPPING_OPTION = 178;

    const MISSING_SHIPPING_CALL_BACK_CONFIGURATION = 179;

    const MISSING_ITEM_SHIPPING_OPTIONS = 180;

    const UNSUPPORTED_PROCESSING_INSTRUCTION = 181;

    const ORDER_COMPLETE_ON_PAYMENT_APPROVAL = 182;

    const BILLING_AGREEMENTS_CANNOT_BE_USED_FOR_SUBSCRIPTIONS = 183;

    const UNSUPPORTED_PAYMENT_SOURCE_FOR_SUBSCRIPTIONS = 184;

    const DUPLICATE_TRANSACTION = 185;

    const REAUTHORIZATION_NOT_SUPPORTED = 186;

    const AUTH_CURRENCY_MISMATCH = 187;

    const REAUTHORIZATION_DECLINED_BY_PROCESSOR = 188;

    const REAUTHORIZATION_NOT_SUPPORTED_FOR_PAYMENT_SOURCE = 189;

    const REFUND_AMOUNT_TOO_LOW = 190;

    const TRANSACTION_DISPUTED = 191;

    const PLATFORM_FEE_EXCEEDED = 192;

    const REFUND_IS_RESTRICTED = 193;
