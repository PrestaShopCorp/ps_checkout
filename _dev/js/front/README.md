...
window.ps_checkout.selectors = {
  ...
  // Notification that will prompt when the user has not accepted all the checkboxes defined with
  // `CONDITIONS_CHECKBOXES`
  NOTIFICATION_CONDITIONS: '.accept-cgv',

  // Notification that will prompt when the user has cancelled the payment
  NOTIFICATION_PAYMENT_CANCELLED: '#ps_checkout-canceled',
  // Notification that will prompt when an error happens while doing a payment
  NOTIFICATION_PAYMENT_ERROR: '#ps_checkout-error',
  // Container for the error extra information if an error happens during a payment.
  NOTIFICATION_PAYMENT_ERROR_TEXT: '#ps_checkout-error-text',

  // Container for all the payment options. This will be used to find all the payment options
  // generated by Ps Checkout
  ...
  // when you change of payment option.
...
```
> The 'payment-option-active' event won't happen until `window.ps_checkout.renderCheckout();` 
